...
// 物理駆動をせず、その場を原点としてオフセット上書きする新しい関数
void zero_instantly(uint8_t index) {
  uint8_t servoID = SERVO_IDS[index];
  
  // 1. その瞬間の物理的な位置をサーボ内部の基準原点（ゼロ点）として上書き強制登録
  hlscl.CalibrationOfs(servoID);
  delay(50); // サーボ内部のEEPROM書き込み待ち
  
  // 2. 上書き完了後のサーボの現在位置（通常は中央値の2048、または0など）を読み出す
  int pos = hlscl.ReadPos(servoID);
  if (pos < 0) pos += 32768; // 負の値（持病）の補正
  uint16_t current_raw = (uint16_t)(pos % 4096);
  
  // 3. この場所をスライダーの「0.0（全開位置 extend_count）」とする
  sd[index].extend_count = current_raw;
  sd[index].servo_direction = MY_DIRECTIONS[index];
  
  // 4. 方向（direction）とリミット（limit）から「1.0（全閉位置 grasp_count）」を自動計算
  int dir = MY_DIRECTIONS[index];
  int limit_width = MY_LIMIT_PULSES[index];
  
  // 原点から指定方向に指定パルス分進んだ場所をターゲットにする
  sd[index].grasp_count = (uint16_t)((current_raw + (dir * limit_width) + 4096) % 4096);
}
